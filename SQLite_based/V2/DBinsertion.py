    ...
    with open('samfile_read.csv','a',encoding='UTF8') as read_file,open('samfile_tags.csv','a',encoding='UTF8') as tags_file,open('samfile_cigar.csv','a',encoding='UTF8') as cigar_file,open('samfile_InsDel.csv','a',encoding='UTF8') as attachment_file: #
        ...
        for read_id,read in enumerate(sam_file.fetch(until_eof=True)):
            ...
            if (read_id>=start) & (read_id<end):
                ...
                pos_shifted=ExplainCigar(read_id,d['seq'],d['qual'],d['ref_pos'],d['cigar'],d['flag'],writer_cigar,writer_attachment)

                writer_reads.writerow([read_id]+list(d.values())+[None,str(pos_shifted)]) 


def ExplainCigar(read_id,seq,qual,pos,cigar,flag,writer_cigar,writer_attachment):
    ...
    if 'D' in letters:
        ...
        for i,del_idx in enumerate(del_idxs):
            seq_insdel=seq_insdel[:del_idx+counter] + '*' + seq_insdel[del_idx+counter:]
            qual_insdel=qual_insdel[:del_idx+counter+1] + qual_insdel[del_idx+counter:]
            ins_count=cigar_extended[:del_idx+counter].count('I')
            pos_ixd=pos+del_idx+counter-1-ins_count
    ...
    if 'I' in letters:
        ...
        for i,ins_idx in enumerate(ins_idxs):
            seq_insdel=seq_insdel[:ins_idx+counter] + seq_insdel[ins_idx+1+counter:] 
            qual_insdel=qual_insdel[:ins_idx+counter] + qual_insdel[ins_idx+1+counter:] 
            pos_ixd=pos+ins_idx+counter-1
            if (i==0) | (ins_idxs[i-1] != ins_idxs[i]-1):
                num=ins_num.pop(0)
                attach='+'+str(num)+(seq[ins_idx+counter:ins_idx+counter+num])
                #if (flag==16):
